...
-- HISTORY:
--     JRK 09/23/86  CREATED ORIGINAL TEST.
--     PWN 02/02/95  REMOVED INCONSISTENCIES WITH ADA 9X.
--     RLB 03/16/07  CORRECTED ILLEGAL (BY AMENDMENT 1) RETURN AND
--                   LEGAL AGGREGATE.

WITH SYSTEM; USE SYSTEM;

PROCEDURE B34006K IS
...
BEGIN
     ...
     P (NULL);                          -- ERROR: NULL.
     P ((TRUE,2,1,"AB",C));             -- OK:    AGGREGATE.
     P (NEW COMPONENT);                 -- ERROR: ALLOCATOR.
     I := X.L;                          -- OK:    SELECTION.
     C (X.C);                           -- OK:    SELECTION.
     Z.S := "ABC";                      -- OK:    SELECTION.
     C (X.ALL);                         -- ERROR: .ALL.
     C (X (1));                         -- ERROR: INDEX.
     P (X (1..1));                      -- ERROR: SLICE.
     P (NOT X);                         -- ERROR: NOT.
