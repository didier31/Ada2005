...
package B393005 is
  ...
  type Leaf_1 is new Branch_1 with null record;                      -- OK
  ...
  -- NOTE: the error associated with T_3B need only be flagged in one of
  --       the two following locations:

  type T_3B is abstract tagged private;
  function Foo(X: T_3B) return Boolean is abstract;         -- OPTIONAL ERROR:
                                          -- full view of type is not abstract

private
  type T_3B is tagged null record;                                   -- ERROR:
