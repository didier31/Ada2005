...
-- PWN 10/27/95  REMOVED CHECK THAT REQUIRED UNIVERSAL_INTEGER
--               TYPE FOR ATTRIBUTE.
...
PROCEDURE B34007H IS

     SUBTYPE COMPONENT IS INTEGER;

     TYPE DESIGNATED IS ARRAY (1 .. 2, 1 .. 3) OF COMPONENT;

     TYPE PARENT IS ACCESS DESIGNATED;

     TYPE T IS NEW PARENT;

     X, Y, Z : T          := NEW DESIGNATED'(OTHERS => (OTHERS => 2));
     W       : PARENT     := NEW DESIGNATED'(OTHERS => (OTHERS => 2));
     C       : COMPONENT  := 1;
     H       : DESIGNATED := (OTHERS => (OTHERS => 1));
     ...
     BEGIN
          RETURN NEW DESIGNATED'(OTHERS => (OTHERS => C));
     END V;
...
BEGIN
     ...
     I := T'FIRST;                      -- ERROR: 'FIRST.
     I := X'FIRST;                      -- OK:    'FIRST.
     I := V'FIRST;                      -- OK:    'FIRST.
     I := T'FIRST (2);                  -- ERROR: 'FIRST (N).
     I := X'FIRST (2);                  -- OK:    'FIRST (N).
     I := V'FIRST (2);                  -- OK:    'FIRST (N).
     I := X'FIRST (0);                  -- ERROR: DIMENSION 0.
     ...
     FOR I IN V'RANGE LOOP              -- OK:    'RANGE.
          NULL;
     END LOOP;
     FOR I IN T'RANGE (2) LOOP          -- ERROR: 'RANGE (N).
          NULL;
     END LOOP;
     ...
     FOR I IN V'RANGE (2) LOOP          -- OK:    'RANGE (N).
          NULL;
     END LOOP;
     FOR I IN X'RANGE (INTEGER'POS (J)) LOOP  -- ERROR: NOT STATIC.
          NULL;
     END LOOP;
