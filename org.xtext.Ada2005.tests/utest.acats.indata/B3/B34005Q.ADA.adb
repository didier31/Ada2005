...
--                   SUPPORTING CODE.
--     RLB 03/16/07  CORRECTED ILLEGAL (BY AMENDMENT 1) RETURNS AND
--                   LEGAL AGGREGATES.

WITH SYSTEM; USE SYSTEM;

PROCEDURE B34005Q IS

     PACKAGE PKG IS
          TYPE LP IS LIMITED PRIVATE;
          PROCEDURE INIT (X : OUT LP; V : INTEGER);
     PRIVATE
     ...
     TYPE PARENT IS ARRAY (1 .. 3) OF COMPONENT;
     ...
     FUNCTION C RETURN COMPONENT IS
     BEGIN
          RETURN X : COMPONENT DO
             INIT (X, 1);
          END RETURN;
     END C;

     PROCEDURE C (X : COMPONENT) IS
     BEGIN
          NULL;
     END C;
...
BEGIN
     ...
     I := X'FIRST;                      -- OK:    'FIRST.
     I := V'FIRST;                      -- OK:    'FIRST.
     I := T'FIRST (1);                  -- OK:    'FIRST (N).
     I := X'FIRST (1);                  -- OK:    'FIRST (N).
     I := V'FIRST (1);                  -- OK:    'FIRST (N).
     ...
     I := X'LAST;                       -- OK:    'LAST.
     I := V'LAST;                       -- OK:    'LAST.
     I := T'LAST (1);                   -- OK:    'LAST (N).
     I := X'LAST (1);                   -- OK:    'LAST (N).
     I := V'LAST (1);                   -- OK:    'LAST (N).
     I := X'LAST_BIT;                   -- ERROR: 'LAST_BIT.
     I := T'LENGTH;                     -- OK:    'LENGTH.
     I := X'LENGTH;                     -- OK:    'LENGTH.
     I := V'LENGTH;                     -- OK:    'LENGTH.
     I := T'LENGTH (1);                 -- OK:    'LENGTH (N).
     I := X'LENGTH (1);                 -- OK:    'LENGTH (N).
     I := V'LENGTH (1);                 -- OK:    'LENGTH (N).
     ...
     FOR I IN V'RANGE LOOP              -- OK:    'RANGE.
          NULL;
     END LOOP;
     FOR I IN T'RANGE (1) LOOP          -- OK:    'RANGE (N).
          NULL;
     END LOOP;
     FOR I IN X'RANGE (1) LOOP          -- OK:    'RANGE (N).
          NULL;
     END LOOP;
     FOR I IN V'RANGE (1) LOOP          -- OK:    'RANGE (N).
          NULL;
     END LOOP;
