...
-- PWN 10/24/95  REMOVED ADDRESS CLAUSE CHECKS NO LONGER LEGAL.
-- WMC 11/08/95  REMOVED OBSOLETE ATTRIBUTES FOR ACVC 2.0.1.
-- PWN 02/01/96  RESTORED ERROR: CHECK IN ADA 95 LEGAL FORMAT.
--               K(6);6.0
-- RLB 03/16/07  CORRECTED ILLEGAL (BY AMENDMENT 1) RETURN.

WITH SYSTEM; USE SYSTEM;

PROCEDURE B34008B IS
...
BEGIN
     ...
     P (C & X);                         -- ERROR: &.
     P (C & C);                         -- ERROR: &.
     A (T'ADDRESS);                     -- ERROR: 'ADDRESS.
                                        -- Not allowed for derived
                                        -- task types.
     A (X'ADDRESS);                     -- OK:    'ADDRESS.
     I := T'AFT;                        -- ERROR: 'AFT.
     B := X'CALLABLE;                   -- OK:    'CALLABLE.
     B := V'CALLABLE;                   -- OK:    'CALLABLE.
     B := T'CONSTRAINED;                -- ERROR: 'CONSTRAINED.
     B := X'CONSTRAINED;                -- ERROR: 'CONSTRAINED.
     I := X'COUNT;                      -- ERROR: 'COUNT.
     I := X.E'COUNT;                    -- ERROR: 'COUNT.
     ...
     I := X'STORAGE_SIZE;               -- OK:    'STORAGE_SIZE.
     P (T'SUCC (X));                    -- ERROR: 'SUCC.
     B := X'TERMINATED;                 -- OK:    'TERMINATED.
     B := V'TERMINATED;                 -- OK:    'TERMINATED.
     P (T'VAL (1));                     -- ERROR: 'VAL.
