...
with B371001_0;
package body B371001_1 is

    type Ptr3 is access all T;
    type Ptr4 is access T;

    -- A dereference of an access type which designates T is not known
    ...
    -- does not apply because there is an ancestor of the type of T that has
    -- a constrained partial view. Thus a discriminant constraint is illegal,
    -- as 3.7.1(7/3) requires the dereference to be known to be constrained.

    P1 : Ptr1 (0) := B371001_0.X (1)'Access; -- ERROR:
    subtype S2 is Ptr2 (2);                  -- ERROR:
    subtype S3 is Ptr3 (3);                  -- ERROR:
    P4 : Ptr4 (4);                           -- ERROR:

begin
    B371001_0.X := (others => (D => 3));
end B371001_1;
