...
PROCEDURE B34007E IS

     SUBTYPE COMPONENT IS INTEGER;

     TYPE DESIGNATED IS ARRAY (1 .. 3) OF COMPONENT;

     TYPE PARENT IS ACCESS DESIGNATED;

     TYPE T IS NEW PARENT;

     X, Y, Z : T          := NEW DESIGNATED'(OTHERS => 2);
     W       : PARENT     := NEW DESIGNATED'(OTHERS => 2);
     C       : COMPONENT  := 1;
     H       : DESIGNATED := (OTHERS => 1);
     ...
     BEGIN
          RETURN NEW DESIGNATED'(OTHERS => C);
     END V;
