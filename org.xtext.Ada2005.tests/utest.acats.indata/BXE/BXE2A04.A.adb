...
--      Shared Passive or Declared Pure library units.  Specifically that 
--      it can not depend on a Remote Types library unit.
--
-- TEST DESCRIPTION: 
--      A Shared Passive library unit is constructed which depends on each of
--      the above.  The dependencies on the Remote Types unit should be 
...
with FXE2A00_2;   -- remote types   - Illegal dependency 

package BXE2A04 is 

   pragma Shared_Passive (BXE2A04);                                  -- ERROR:
   ...
   Color_Set_2 : FXE2A00_2.Type_From_2;    -- Illegal: remote types
